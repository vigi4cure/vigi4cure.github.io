...
voronoiMap = function(map, url, initialSelections) {
  ...
  //set proper edges
   var edges = [
    map.latLngToLayerPoint(box[0]),
    map.latLngToLayerPoint(box[1])
  ];

  var voronoi = d3.geom.voronoi()
      .clipExtent([[edges[0].x, edges[0].y],[edges[1].x,edges[1].y]])
  ...
  var draw = function() {
    ...
    var svgPoints = g.attr("class", "points")
      .selectAll("g")
        .data(filteredPoints.filter(function(d) {return d.cell.length}))
      .enter().append("g")
        .attr("class", "point");
